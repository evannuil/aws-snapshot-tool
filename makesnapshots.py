...
# version 1.5: Select volumes for snapshotting depending on Tag and not from
#              config file
...
# version 3.1: Fix a bug with the deletelist and added a pause in the
#              volume loop
# version 3.2: Tags of the volume are placed on the new snapshot
# version 3.3: Merged IAM role addidtion from Github
# version 3.4: Linted and updated code for python 2/3 compatibility
...
vols = conn.get_all_volumes(filters={'tag:' +
...
for vol in vols:
    try:
        ...
        description = '%(period)s_snapshot \
            %(vol_id)s_%(period)s_%(date_suffix)s \
            by snapshot script at %(date)s' % {
            'period': period,
            'vol_id': vol.id,
        ...
        try:
            ...
            suc_message = 'Snapshot created with description: %s and tags: %s' \
...
result = '\nFinished making snapshots at %(date)s with \
        %(count_success)s snapshots of %(count_total)s possible.\n\n' % {
    'date': datetime.today().strftime('%d-%m-%Y %H:%M:%S'),
...
if sns_arn:
    if errmsg:
        sns.publish(sns_arn, 'Error in processing volumes: '
